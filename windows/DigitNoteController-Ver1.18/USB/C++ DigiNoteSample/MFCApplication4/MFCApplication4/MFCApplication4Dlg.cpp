...
typedef int(WINAPI *PdisconnectDevice)();
...
int ThreadFunc(LPVOID pParam)
{
	...
	x = x * width / 21000; y = y * height / 29700;
...
{
	hDll = LoadLibrary(L"DigitNoteUSBController.dll");
	...
	disconnectDevice = (PdisconnectDevice)GetProcAddress(hDll, "_disconnectDevice@0");
...
void CMFCApplication4Dlg::OnBnClickedButton12()
{
	...
	char buf[1024] = { 0 };
	snprintf(buf, sizeof(buf), "ret:%d deviceName:%s", ret, deviceName);
	CString cstr(buf);
	pEditContralWin->SetWindowTextW(cstr);
	//SetDlgItemText(IDC_EDIT1, str);
	// TODO: 在此添加控件通知处理程序代码
